...
| **Boundary state tracking** *(new in 4.17.2)* | `BoundaryStateTracker<TState>` &mdash; third storage primitive on the recorder shelf alongside `SequenceRecorder<T>` (durable ordered) and `KeyedRecorder<T>` (durable 1:1). Tracks live transient state DURING a `[start, stop]` event interval (LLM stream partial, tool args streaming, agent turn state) and clears on stop. O(1) reads via `getActive` / `hasActive` / `activeCount`. Mental model: existing recorder *interfaces* are observers; storage primitives are *bookkeeping shelves*. A real recorder picks ONE observer interface AND ONE storage shelf via `extends + implements`. [Guide →](https://footprintjs.github.io/footPrint/guides/features/recorder-storage-primitives/) |
