...
describe('Scenario: write → commit → read', () => {
  it('stage writes are visible after commit', () => {
    ...
    const ctx = new StageContext(runId, 'validate', 'validate', mem, '', log);
  ...
  it('stage writes are visible to the next stage after commit', () => {
    const { mem, log, runId } = createRun();
    const stage1 = new StageContext(runId, 'stage1', 'stage1', mem, '', log);
    stage1.setObject([], 'counter', 1);
    stage1.commit();

    const stage2 = stage1.createNext(runId, 'stage2', 'stage2');
  ...
  it('uncommitted writes are NOT visible to another stage', () => {
    ...
    const stage2 = new StageContext(runId, 'stage2', 'stage2', mem, '', log);
    expect(stage2.getValue([], 'secret')).toBeUndefined();
  });

  it('read-after-write within same stage sees uncommitted writes', () => {
    const { mem, log, runId } = createRun();
    const ctx = new StageContext(runId, 'stage1', 'stage1', mem, '', log);
  ...
  it('multiple commits accumulate state', () => {
    ...
    const s1 = new StageContext(runId, 's1', 's1', mem, '', log);
    s1.setObject([], 'a', 1);
    s1.commit();

    const s2 = s1.createNext(runId, 's2', 's2');
    s2.setObject([], 'b', 2);
    s2.commit();

    const s3 = s2.createNext(runId, 's3', 's3');
    s3.setObject([], 'c', 3);
    s3.commit();
