...
describe('Property 4: History Growth Invariant', () => {
  /**
   * Feature: scope-recorder-pattern
   * Property 4: History Growth Invariant
   * **Validates: Requirements 2.1**
   *
   * For any Scope instance, after N commit operations, the snapshots array
   * SHALL contain exactly N entries.
   */
  ...
  test('after N commits, snapshots array contains exactly N entries', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 20 }), // number of commits
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - snapshots array should have exactly N entries
          expect(scope.getSnapshots().length).toBe(numCommits);
        }
      ),
      { numRuns: 100 }
    );
  ...
  test('each commit increments snapshot count by exactly one', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 15 }), // number of commits
        (pipelineId, stageName, numCommits) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
          ...
          for (let i = 0; i < numCommits; i++) {
            ...
            // Each commit should add exactly one snapshot
            expect(countAfter).toBe(countBefore + 1);
            expect(countAfter).toBe(i + 1);
          }
        }
      ),
      { numRuns: 100 }
  ...
  test('snapshot count equals commit count regardless of write operations per commit', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.array(
          fc.integer({ min: 1, max: 10 }), // writes per commit
          { minLength: 1, maxLength: 10 }
        ),
        (pipelineId, stageName, writesPerCommit) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
            pipelineId,
          ...
          // Act - perform commits with varying numbers of writes
          ...
          // Assert - snapshot count should equal number of commits, not writes
          expect(scope.getSnapshots().length).toBe(writesPerCommit.length);
        }
      ),
      { numRuns: 100 }
    );
    ...
    fc.assert(
      fc.property(
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - snapshot count should equal number of commits
          expect(scope.getSnapshots().length).toBe(numCommits);
        }
      ),
      { numRuns: 100 }
  ...
  test('empty commits (no writes) still create snapshots', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 20 }), // number of empty commits
        (pipelineId, stageName, numCommits) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
          ...
          // Act - perform N empty commits (no setValue before commit)
          for (let i = 0; i < numCommits; i++) {
            scope.commit();
          }

          // Assert - snapshots array should still have exactly N entries
          expect(scope.getSnapshots().length).toBe(numCommits);
        }
      ),
      { numRuns: 100 }
  ...
  test('snapshot indices are sequential from 0 to N-1', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - snapshot indices should be sequential
          const snapshots = scope.getSnapshots();
          for (let i = 0; i < numCommits; i++) {
            expect(snapshots[i].index).toBe(i);
          }
        }
      ),
      { numRuns: 100 }
  ...
  test('getCurrentSnapshotIndex equals N-1 after N commits', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - getCurrentSnapshotIndex should be N-1
          expect(scope.getCurrentSnapshotIndex()).toBe(numCommits - 1);
        }
      ),
      { numRuns: 100 }
    );
  ...
  test('initial scope has zero snapshots', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        (pipelineId, stageName) => {
          ...
          // Assert - no commits means no snapshots
          expect(scope.getSnapshots().length).toBe(0);
...
describe('Property 5: Time-Travel Retrieval', () => {
  /**
   * Feature: scope-recorder-pattern
   * Property 5: Time-Travel Retrieval
   * **Validates: Requirements 2.2, 2.3**
   *
   * For any Scope instance with N snapshots, calling getStateAt with index i
   * (where 0 ≤ i < N) SHALL return the state as it existed after the i-th commit.
   */
  ...
  test('getStateAt(i) returns state as it existed after i-th commit', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.array(arbPrimitive, { minLength: 2, maxLength: 10 }),
  ...
  test('getStateAt returns exact state at each historical point with multiple keys', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, commits) => {
          ...
          const expectedStates: Record<string, unknown>[] = [];
          ...
          for (const { key, value } of commits) {
            ...
            // Store a copy of the accumulated state at this point
  ...
  test('getStateAt returns undefined for out-of-bounds indices', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 10 }), // number of commits
        fc.integer({ min: 1, max: 100 }), // offset beyond bounds
        (pipelineId, stageName, numCommits, offset) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
            pipelineId,
          ...
          // Assert - out of bounds indices return undefined
          expect(scope.getStateAt(-1)).toBeUndefined();
          expect(scope.getStateAt(-offset)).toBeUndefined();
          expect(scope.getStateAt(numCommits)).toBeUndefined();
          expect(scope.getStateAt(numCommits + offset)).toBeUndefined();
        }
      ),
      { numRuns: 100 }
    );
  ...
  test('getStateAt returns correct state for any valid index in range [0, N-1]', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 2, max: 15 }), // number of commits
        (pipelineId, stageName, numCommits) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
          ...
          // Track values at each commit
          const valuesAtCommit: number[] = [];
          ...
          // Assert - verify every valid index returns correct state
          for (let i = 0; i < numCommits; i++) {
            const state = scope.getStateAt(i);
            expect(state).toBeDefined();
  ...
  test('getStateAt preserves state evolution across commits', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
  ...
  test('getStateAt returns independent copies for each index', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 2, max: 10 }), // number of commits
        (pipelineId, stageName, numCommits) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
          ...
          const states = [];
          for (let i = 0; i < numCommits; i++) {
            states.push(scope.getStateAt(i));
          }
  ...
  test('getStateAt called multiple times for same index returns consistent results', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 10 }), // number of commits
        fc.integer({ min: 2, max: 5 }), // number of times to call getStateAt
        (pipelineId, stageName, numCommits, numCalls) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
            pipelineId,
          ...
          // Assert - multiple calls to getStateAt for same index return equal results
          for (let idx = 0; idx < numCommits; idx++) {
            const results = [];
            for (let call = 0; call < numCalls; call++) {
              results.push(scope.getStateAt(idx));
            }

            // All results should be equal
            for (let call = 1; call < numCalls; call++) {
              expect(results[call]).toEqual(results[0]);
            }
          }
        }
      ),
...
describe('Property 6: Time-Travel Immutability', () => {
  /**
   * Feature: scope-recorder-pattern
   * Property 6: Time-Travel Immutability
   * **Validates: Requirements 2.4**
   *
   * For any Scope instance, calling getStateAt SHALL NOT modify the current
   * state—the state before and after the call SHALL be identical.
   */
  ...
  test('getStateAt does not modify current state', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
  ...
  test('getStateAt does not modify snapshot history', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Get snapshot count before time-travel
          ...
          expect(snapshotCountAfter).toEqual(snapshotCountBefore);
        }
      ),
      { numRuns: 100 }
    );
  ...
  test('modifying returned state does not affect scope', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Get state at first commit
          const historicalState = scope.getStateAt(0);

          // Attempt to modify the returned state
          if (historicalState) {
            (historicalState as Record<string, unknown>).data = { value: 'MODIFIED' };
            (historicalState as Record<string, unknown>).newKey = 'INJECTED';
          }
  ...
  test('modifying returned state does not affect other snapshots', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Get state at first commit and modify it
          const state0 = scope.getStateAt(0);
          if (state0) {
            (state0 as Record<string, unknown>).data = { value: 'MODIFIED' };
          }
  ...
  test('getStateAt with nested objects does not modify current state', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        arbNestedObject,
        arbNestedObject,
        (pipelineId, stageName, obj1, obj2) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
            pipelineId,
          ...
          // Attempt to modify nested properties in returned state
          if (historicalState?.config) {
            const config = historicalState.config as Record<string, unknown>;
            config.settings = { MODIFIED: true };
          }
  ...
  test('multiple getStateAt calls do not accumulate side effects', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 10 }),
        fc.integer({ min: 5, max: 20 }), // number of getStateAt calls
        (pipelineId, stageName, numCommits, numCalls) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
          ...
          for (let call = 0; call < numCalls; call++) {
            const idx = call % numCommits;
            const state = scope.getStateAt(idx);
            // Attempt to modify each returned state
            if (state) {
              (state as Record<string, unknown>).modified = call;
            }
          }
  ...
  test('getStateAt with out-of-bounds index does not modify state', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits, offset) => {
          ...
          scope.getStateAt(-1);
          scope.getStateAt(-offset);
          scope.getStateAt(numCommits);
  ...
  test('getCurrentSnapshotIndex is unchanged after getStateAt calls', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Get current snapshot index before time-travel
          const indexBefore = scope.getCurrentSnapshotIndex();
          ...
          // Current snapshot index should be unchanged
          const indexAfter = scope.getCurrentSnapshotIndex();
          expect(indexAfter).toEqual(indexBefore);
          expect(indexAfter).toEqual(numCommits - 1);
        }
      ),
      { numRuns: 100 }
    );
...
describe('Property 7: Snapshot Metadata Completeness', () => {
  /**
   * Feature: scope-recorder-pattern
   * Property 7: Snapshot Metadata Completeness
   * **Validates: Requirements 2.5**
   *
   * For any snapshot in the Scope's history, it SHALL contain non-empty stageName,
   * non-empty pipelineId, and a valid timestamp.
   */
  ...
  test('every snapshot has non-empty stageName, pipelineId, and valid timestamp', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - verify all snapshots have complete metadata
          const snapshots = scope.getSnapshots();
          for (const snapshot of snapshots) {
            // stageName must be non-empty
            expect(snapshot.stageName).toBeTruthy();
            expect(typeof snapshot.stageName).toBe('string');
            expect(snapshot.stageName.length).toBeGreaterThan(0);

            // pipelineId must be non-empty
            expect(snapshot.pipelineId).toBeTruthy();
            expect(typeof snapshot.pipelineId).toBe('string');
            expect(snapshot.pipelineId.length).toBeGreaterThan(0);
  ...
  test('snapshot stageName matches the scope stageName', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - all snapshots should have the correct stageName
          const snapshots = scope.getSnapshots();
          for (const snapshot of snapshots) {
            expect(snapshot.stageName).toBe(stageName);
          }
        }
      ),
      { numRuns: 100 }
  ...
  test('snapshot pipelineId matches the scope pipelineId', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - all snapshots should have the correct pipelineId
          const snapshots = scope.getSnapshots();
          for (const snapshot of snapshots) {
            expect(snapshot.pipelineId).toBe(pipelineId);
          }
        }
      ),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - snapshot indices should be sequential from 0
          const snapshots = scope.getSnapshots();
          for (let i = 0; i < numCommits; i++) {
            expect(snapshots[i].index).toBe(i);
          }
  ...
  test('snapshot timestamps are monotonically non-decreasing', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
  ...
  test('snapshot timestamps are reasonable (within expected range)', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Assert - all timestamps should be within the test execution window
          const snapshots = scope.getSnapshots();
          for (const snapshot of snapshots) {
            expect(snapshot.timestamp).toBeGreaterThanOrEqual(timeBefore);
            expect(snapshot.timestamp).toBeLessThanOrEqual(timeAfter);
          }
        }
      ),
      { numRuns: 100 }
  ...
  test('empty commits still have complete metadata', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName, numCommits) => {
          ...
          // Act - perform empty commits (no setValue before commit)
          for (let i = 0; i < numCommits; i++) {
            scope.commit();
          }

          // Assert - all snapshots should still have complete metadata
          const snapshots = scope.getSnapshots();
          expect(snapshots.length).toBe(numCommits);
  ...
  test('snapshot state is a valid object', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        fc.integer({ min: 1, max: 10 }),
        arbPrimitive,
        (pipelineId, stageName, numCommits, value) => {
          // Arrange
          const globalStore = new GlobalStore();
          const scope = new Scope({
            pipelineId,
          ...
          // Assert - all snapshots should have a valid state object
          const snapshots = scope.getSnapshots();
          for (const snapshot of snapshots) {
  ...
  test('metadata is preserved across multiple scopes with same pipelineId', () => {
    fc.assert(
      fc.property(
        arbPipelineId,
        arbStageName,
        ...
        (pipelineId, stageName1, stageName2, numCommits) => {
          // Ensure different stage names
          fc.pre(stageName1 !== stageName2);
          ...
          // Assert - each scope's snapshots have correct metadata
          const snapshots1 = scope1.getSnapshots();
          const snapshots2 = scope2.getSnapshots();

          for (const snapshot of snapshots1) {
            expect(snapshot.stageName).toBe(stageName1);
            expect(snapshot.pipelineId).toBe(pipelineId);
            expect(snapshot.timestamp).toBeGreaterThan(0);
          }

          for (const snapshot of snapshots2) {
            expect(snapshot.stageName).toBe(stageName2);
            expect(snapshot.pipelineId).toBe(pipelineId);
            expect(snapshot.timestamp).toBeGreaterThan(0);
          }
        }
