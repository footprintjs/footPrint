...
import { isStageNodeReturn, StageNode } from '../../src/core/executor/Pipeline';
...
describe('Dynamic StageNode Execution Property Tests', () => {
  ...
  const { Pipeline } = require('../../src/core/executor/Pipeline');
  const { StageNode, PipelineStageFunction } = require('../../src/core/executor/types');
    ...
    it('should commit parent patch before executing dynamic children', async () => {
      await fc.assert(
        fc.asyncProperty(stageNameArb, async (rootName) => {
          ...
          const mockContext = {
            commit: () => commitOrder.push('parent-commit'),
            addLog: () => {},
            addError: () => {},
            createChild: () => ({
              commit: () => commitOrder.push('child-commit'),
              addLog: () => {},
              addError: () => {},
              createNext: () => mockContext,
            }),
            createNext: () => mockContext,
          };
