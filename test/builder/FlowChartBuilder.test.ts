...
describe('FlowChartBuilder — toSpec() & specToStageNode()', () => {
  test('toSpec emits pure JSON with hasDecider/branchIds; specToStageNode reconstructs shape', () => {
    const fb = new FlowChartBuilder().start('init').addListOfFunction([
      {
        id: 'conversation',
        name: 'retriever',
        build: (b) =>
          b
            .addDecider(() => 'intent')
            .addFunctionBranch('intent', 'identifyIntent', undefined, (bb) =>
              bb.addFunction('buildPrompt').addFunction('askLlm'),
            )
