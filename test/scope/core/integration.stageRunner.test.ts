...
class FakeStageContext implements StageContextLike {
    ...
    addDebugInfo(key: string, val: unknown) { this.calls.push({ op: 'addDebugInfo', args: [key, val] }); }
    addErrorInfo(key: string, val: unknown) { this.calls.push({ op: 'addErrorInfo', args: [key, val] }); }
    getFromGlobalContext(key: string) { this.calls.push({ op: 'getFromGlobalContext', args: [key] }); return `root:${key}`; }
    setRoot(key: string, value: unknown) { this.calls.push({ op: 'setRoot', args: [key, value] }); this.store[`root.${key}`] = value; }
}
...
describe('integration: stage runner + toScopeFactory', () => {
    ...
    test('factory input: stage receives whatever the factory returns', () => {
        ...
        const { ctx, scope } = runStage(factory, (s) => {
            s.setObject(['x'], 'y', 5);
