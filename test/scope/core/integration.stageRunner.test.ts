...
class FakeStageContext implements StageContextLike {
  ...
  addDebugInfo(key: string, val: unknown) {
    this.calls.push({ op: 'addDebugInfo', args: [key, val] });
  }

  addErrorInfo(key: string, val: unknown) {
    this.calls.push({ op: 'addErrorInfo', args: [key, val] });
  }
