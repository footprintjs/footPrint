...
import { defineScopeSchema, defineScopeFromZod } from '../../../../src/scope/state/zod';
...
describe('defineScopeFromZod', () => {
    ...
    test('strict=deny throws on invalid writes; strict=warn skips write; strict=off allows write', () => {
        const denyFactory = defineScopeFromZod(
            defineScopeSchema({ n: z.number().int() }),
            { strict: 'deny' }
        );
        const warnFactory = defineScopeFromZod(
            defineScopeSchema({ n: z.number().int() }),
            { strict: 'warn' }
        );
        const offFactory  = defineScopeFromZod(
            defineScopeSchema({ n: z.number().int() }),
            { strict: 'off' }
        );
