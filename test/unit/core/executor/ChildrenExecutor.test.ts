...
import { ChildrenExecutor, ExecuteNodeFn } from '../../../../src/core/executor/handlers/ChildrenExecutor';
...
describe('ChildrenExecutor', () => {
  ...
  describe('executeSelectedChildren', () => {
    ...
    it('should add debug info for selected and skipped children', async () => {
      ...
      const parentContext = ctx.pipelineRuntime.rootStageContext;
      const addLogSpy = jest.spyOn(parentContext, 'addLog');
      ...
      expect(addLogSpy).toHaveBeenCalledWith('selectedChildIds', ['child1']);
      expect(addLogSpy).toHaveBeenCalledWith('selectorPattern', 'multi-choice');
      expect(addLogSpy).toHaveBeenCalledWith('skippedChildIds', ['child2', 'child3']);
    });
