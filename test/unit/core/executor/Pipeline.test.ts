...
import { Pipeline } from '../../../../src/core/executor';
...
class FakeStageContext {
  ...
  commit() {
    this.calls.push({ op: 'commit', args: [] });
  }
  ...
  createDeciderContext(_path: string, _role: string) {
    return this.createDecider(_path, _role);
  }
  ...
  createNextContext(_path: string, name: string) {
    return this.createNext(_path, name);
  }
  ...
  createChildContext(_pipelineId: string, id: string, name: string) {
    return this.createChild(_pipelineId, id, name);
  }
}
...
describe('Linear & break semantics', () => {
  ...
  test('Break in parent stage skips children and next', async () => {
    const fns = {
      PARENT: jest.fn((_s: any, breakFn: () => void) => {
        breakFn();
      }),
      A: jest.fn(),
      NEXT: jest.fn(),
...
describe('Validation & error paths', () => {
  ...
  test('Stage throws → commits patch, records error, rethrows', async () => {
    ...
    // Spot check commit was invoked
    const ctx = (p as any).pipelineRuntime.rootStageContext as FakeStageContext;
    expect(ctx.calls.some((c) => c.op === 'commit')).toBe(true);
  });
});
...
describe('Fork-only aggregation & throttling', () => {
  ...
  test('Returns children bundle; flags throttled child via throttlingErrorChecker', async () => {
    ...
    const flagged = childTh?.calls.some(
      (c) => c.op === 'updateObject' && c.args[0][0] === 'monitor' && c.args[1] === 'isThrottled' && c.args[2] === true,
    );
...
describe('Debug breadcrumbs & helpers', () => {
  ...
  test('Adds debug info for children order and total count in fork+next', async () => {
    ...
    const ctx = (p as any).pipelineRuntime.rootStageContext as FakeStageContext;
    const orders = ctx.calls.filter((c) => c.op === 'addLog' && c.args[0] === 'orderOfExecution');
    const totals = ctx.calls.filter((c) => c.op === 'addLog' && c.args[0] === 'totalChildren');
