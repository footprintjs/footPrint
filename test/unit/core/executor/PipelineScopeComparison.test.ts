...
import type { PipelineStageFunction } from '../../../../src/core/executor';
import { StageNode, Pipeline } from '../../../../src/core/executor';
...
function createStageMap(): Map<string, PipelineStageFunction<any, any>> {
  return new Map(Object.values(PipelineStages).map((stage) => [stage, jest.fn((input) => input)]));
}
...
describe.each(variants)('Pipeline – $label scope', ({ scopeFactory }) => {
  ...
  it('handles dynamic decider function', async () => {
    await pipeline.execute();
    expect(stageMap.get(PipelineStages.IDENTIFY_INTENT)).toHaveBeenCalled();
    expect(stageMap.get(PipelineStages.ASK_LLM)).not.toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ handler: 'alternativeClaude' }),
    );
  });
});
...
describe('Pipeline – throttlingErrorChecker', () => {
  ...
  function buildPipeline(
    ...
    Object.entries(childStages).forEach(([name, fn]) => stageMap.set(name as any, jest.fn(fn)));
