...
import { StageNode } from '../../../src/core/pipeline';
import { PipelineStageFunction } from '../../../src/core/pipeline';
...
describe('TreePipeline – Embedded Stage Functions', () => {
    ...
    it('fork+next (embedded fns) runs parent BEFORE children and children BEFORE next', async () => {
        ...
        const root: StageNode<any, PipelineScope> = {
            name: 'PARENT',
            fn: jest.fn(() => { calls.push('parent'); }),
            children: [
                { id: 'x', name: 'X', fn: jest.fn(() => { calls.push('childX'); }) },
                { id: 'y', name: 'Y', fn: jest.fn(() => { calls.push('childY'); }) },
            ],
            next: {
                name: 'NEXT',
                fn: jest.fn(() => { calls.push('next'); }),
            },
        };
