...
import type { StageNode, PipelineStageFunction } from '../../../src/core/pipeline';
...
describe('Streaming Support Property Tests', () => {
    describe('Property 1: Streaming flag propagation', () => {
        ...
        const stageNameArb = fc.string({ minLength: 1, maxLength: 50 })
        ...
        it('should inject a defined streamCallback for streaming stages', async () => {
            await fc.assert(
                fc.asyncProperty(stageNameArb, streamIdArb, async (stageName, maybeStreamId) => {
                    let receivedCallback: ((token: string) => void) | undefined = undefined;
    ...
    describe('Property 4: Token delivery with correct streamId (FlowBuilder API)', () => {
        ...
        const streamIdArb = fc.string({ minLength: 1, maxLength: 20 })
            .filter(s => /^[a-zA-Z][a-zA-Z0-9]*$/.test(s));
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (stage1Name, stage2Name, streamId1, streamId2, tokens1, tokens2) => {
                        ...
                        const stage1TokensCorrect = stage1Tokens.length === tokens1.length &&
                            stage1Tokens.every((r, i) => r.token === tokens1[i]);

                        // Verify tokens from stage 2
                        const stage2Tokens = receivedTokens.filter(r => r.streamId === uniqueStreamId2);
                        const stage2TokensCorrect = stage2Tokens.length === tokens2.length &&
                            stage2Tokens.every((r, i) => r.token === tokens2[i]);
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (stageName, streamId, tokenNumbers) => {
                        ...
                        const orderPreserved = receivedTokens.length === tokens.length &&
                            receivedTokens.every((token, i) => token === tokens[i]);
