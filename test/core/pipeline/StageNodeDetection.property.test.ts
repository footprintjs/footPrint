...
describe('Backward Compatibility Property Tests', () => {
  ...
  describe('Property 11: Backward Compatibility', () => {
    it('should execute legacy DynamicChildrenResult pattern correctly', async () => {
      await fc.assert(
        fc.asyncProperty(stageNameArb, async (rootName) => {
          const executedChildren: string[] = [];

          // Root stage returns legacy DynamicChildrenResult
          const rootStage = () => ({
            isDynamic: true,
            ...
            dynamicChildren: [
              {
                ...
                id: 'legacy-child-1',
                fn: () => {
                  executedChildren.push('legacyChild1');
                  return 'legacy-result-1';
                },
              },
              {
                name: 'legacyChild2',
                id: 'legacy-child-2',
                fn: () => {
                  executedChildren.push('legacyChild2');
                  return 'legacy-result-2';
                },
              },
            ],
          });
          ...
          // Both legacy children should have executed
          const bothExecuted =
            executedChildren.includes('legacyChild1') && executedChildren.includes('legacyChild2');

          // Result should have entries for both children
          const hasResults = result['legacy-child-1'] !== undefined && result['legacy-child-2'] !== undefined;

          return bothExecuted && hasResults;
        }),
        { numRuns: 50 },
      );
    });

    it('should prefer StageNode pattern over legacy when both could match', async () => {
      await fc.assert(
        fc.asyncProperty(stageNameArb, async (rootName) => {
          const executedChildren: string[] = [];

          // This object has both 'name' + 'children' (StageNode pattern)
          // AND could be interpreted as DynamicChildrenResult if we checked isDynamic
          // But since it has 'name' + 'children', it should be treated as StageNode
          const rootStage = () => ({
            name: 'stageNodePattern',
            children: [
              {
                name: 'child1',
                id: 'child-1',
          ...
          // Child should have executed via StageNode pattern
          return executedChildren.includes('child1');
        }),
        { numRuns: 50 },
      );
    });
