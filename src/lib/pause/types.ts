...
export class PauseSignal extends Error {
  ...
  prependSubflow(subflowId: string): void {
    this.subflowPath.unshift(subflowId);
  }
}
...
export interface PausableHandler<TScope = any, TInput = unknown> {
  /** First-run phase. Return { pause: true } to pause, or void to continue normally. */
  execute: (scope: TScope) => Promise<PauseResult | void> | PauseResult | void;
