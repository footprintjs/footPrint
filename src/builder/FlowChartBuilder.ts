...
export class DeciderList<TOut = any, TScope = any> {
     ...
     */
    addBranchList(
        branches: Array<{ id: string; name: string; fn?: PipelineStageFunction<TOut, TScope>; build?: (b: FlowChartBuilder<TOut, TScope>) => void }>
    ): DeciderList<TOut, TScope> {
...
export class FlowChartBuilder<TOut = any, TScope = any> {
    ...
    start(functionName: string, fn?: PipelineStageFunction<TOut, TScope>, id?: string): this {
        ...
        n.name = functionName;
        if (id) n.id = id;
        if (fn) { n.fn = fn; this._addToMap(n.name, fn); }
        this._root = n;
        this._cursor = n;
        return this;
    }
    ...
    private _needCursor() { return this._cursor ?? fail(`cursor undefined; call start() first`); }
