import { z, type ZodTypeAny, type ZodRecord } from "zod";
...
export function unwrap(schema: ZodTypeAny | null | undefined): ZodTypeAny | null {
    ...
    while (isZodNode(s)) {
        ...
        if (isZodNode(def.innerType)) { s = def.innerType; continue; } // default/optional/nullable
        if (isZodNode(def.schema))    { s = def.schema;    continue; } // effects/branded/catch
        if (isZodNode(def.type))      { s = def.type;      continue; } // readonly
        break;
    }
