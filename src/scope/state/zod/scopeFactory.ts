...
import { unwrap, getRecordValueType, parseWithThis, isZodNode } from "./utils/validateHelper";
...
function validateOnWrite(
    ...
    try {
        parseWithThis(schema, value); // throws on invalid
        return true;
    } catch (err) {
...
function analyze(schema: ZodTypeAny): Node {
    ...
    if (base instanceof z.ZodObject) {
        ...
        const shapeObj: Record<string, ZodTypeAny> =
...
const readAt = <T>(ctx: StageContextLike, path: string[], key?: string) =>
    ctx.getValue(path, key) as T | undefined;
        ...
        case "object": {
            ...
            return new Proxy({}, {
                get(target, prop: string | symbol) {
                    ...
                    if (prop in target) return Reflect.get(target as any, prop as any);

                    // 0.a) meta / probe props that must NOT throw
                    if (prop === "then") return undefined;                  // avoid thenable assimilation (await)
                    if (prop === "asymmetricMatch") return undefined;       // Jest asymmetric matcher probe
                    if (prop === "constructor") return Object;              // harmless default
                    if (prop === Symbol.toStringTag) return "ScopeProxy";   // nicer tooling label
                    ...
                    if (!node.fields || !(prop in node.fields)) {
