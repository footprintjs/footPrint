...
import type { ScopeFactory } from '../context/types';
import type { PipelineRuntime, ContextTreeType } from '../context/PipelineRuntime';
...
export class FlowChartExecutor<TOut = any, TScope = any> {
  ...
  /**
   * Execute the flowchart (alias for run()).
   * Provided for backward compatibility with Pipeline.execute().
   *
   * @returns Promise resolving to the execution result
