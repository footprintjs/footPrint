...
## defineContract (explicit approach)

For cases where you want to separate schema definitions from the builder, use `defineContract()`.

**Note on property names:** The builder's `.contract()` method uses `input`/`output`/`mapper` (short form). `defineContract()` uses the longer `inputSchema`/`outputSchema`/`outputMapper` for clarity at the call site. Both produce the same result.

```ts
import { defineContract } from 'footprintjs';
import { z } from 'zod';
...
const spec = contract.toOpenAPI({ title: 'Order API', version: '2.0.0' });

// Execute and extract typed output
const executor = new FlowChartExecutor(chart);
await executor.run({ input: { item: 'Widget', quantity: 3, unitPrice: 29.99 } });
...
Both `.contract()` and `defineContract()` accept plain JSON Schema objects if you don't use Zod:
