...
|--------|---------|-------------|
| `title` | First line of chart description (else `'API'`) | API title |
| `version` | `'1.0.0'` | API version string |
| `description` | Chart description | API description |
| `path` | `'/<slugified-root-stage-id>'` | HTTP path |
...
The `name` is derived from the root stage **id** (here `'receive-order'`), sanitized to the MCP tool-name allowlist `[A-Za-z0-9_\-.]` — any other character is replaced with an underscore. The `description` is the numbered step list generated from stage descriptions you provide in the builder. When no contract declares an `input` schema, `inputSchema` defaults to the MCP no-parameter form `{ type: 'object', properties: {}, additionalProperties: false }`.
