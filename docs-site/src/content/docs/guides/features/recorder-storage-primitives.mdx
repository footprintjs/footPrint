...
<Aside type="caution">
**v5 composition model — stores, not base classes.** Under the "one purpose per recorder" rule, the storage shelf ships as **concrete stores** you own as a field (`new SequenceStore<T>()`, `new KeyedStore<T>()`, `new BoundaryStateStore<TState>()`) and compose with the observer interface you implement. The abstract base classes `SequenceRecorder<T>` / `KeyedRecorder<T>` / `BoundaryStateTracker<TState>` were **removed in 7.0.0** — there is no inheritance path; composing the concrete stores is the only model. The examples below show that pattern.
</Aside>
