...
This guide assumes you know how to build a flowchart — see [Building a flowchart](/footPrint/guides/building-blocks/stages/) first.

footprintjs observes runs through pluggable recorder channels. The two you reach for most: **Scope recorders** (`attachScopeRecorder`) fire on data operations (reads, writes, commits) *during* stage execution. **Flow recorders** (`attachFlowRecorder`) fire on control-flow events (decisions, loops, forks) *after* each stage completes. A third **Emit channel** (`attachEmitRecorder`) carries consumer-emitted structured events from `scope.$emit(...)`, and a build-time **`StructureRecorder`** observes chart shape during construction.

All recorders attach to the executor and run in error-isolated dispatch — one failing recorder never stops the pipeline. A `CombinedRecorder` can implement hooks across the scope, flow, and emit channels at once and attach via a single `attachCombinedRecorder(...)` call — that's what `executor.enableNarrative()` uses internally.
...
Implement `ScopeRecorder` to build audit logs, compliance trails, or custom telemetry. All hooks are optional:

```ts
import { flowChart, FlowChartExecutor, type ScopeRecorder, type ReadEvent, type WriteEvent, type CommitEvent } from 'footprintjs';
...
When a pipeline loops many times, the default narrative can grow large. footprintjs ships several loop-aware strategies — `NarrativeFlowRecorder` (default), `WindowedNarrativeFlowRecorder`, `SilentNarrativeFlowRecorder`, `AdaptiveNarrativeFlowRecorder`, `SeparateNarrativeFlowRecorder`, `MilestoneNarrativeFlowRecorder`, `ProgressiveNarrativeFlowRecorder`, and `RLENarrativeFlowRecorder`:
...
- [Strategy Comparison](https://footprintjs.github.io/footprint-playground/samples/strategy-comparison) — loop strategies side by side
