...
Call `executor.setRedactionPolicy()` before `run()`. One config covers four dimensions:
...
| `fields` | `{ patient: ['ssn', 'dob'] }` | Specific nested paths within objects (dot-notation supported) |
| `emitPatterns` | `[/\.auth\./, /\.billing\./]` | Regex against `scope.$emit()` event names — matched payloads become `[REDACTED]` |
...
console.log(report.patterns);         // ['password|token|secret'] — regex source strings, not RegExp
```

## Subflow redaction

Redaction policies propagate to subflows automatically. See [examples/runtime-features/redaction/02-subflow.ts](https://github.com/footprintjs/footPrint/blob/main/examples/runtime-features/redaction/02-subflow.ts) for a working example.
