...
The matching `outputMapper` option maps the subflow's final state back into the parent scope. Top-level array keys are **concatenated** by default (`[...parent, ...subflow]`) — return only the delta items, or set `arrayMerge: ArrayMergeMode.Replace` (from `footprintjs/advanced`) to overwrite instead. Scalars are always replaced.

```ts
.addSubFlowChartNext('sf-payment', paymentSubflow, 'Payment', {
  inputMapper: (s: any) => ({ amount: s.amount, cardLast4: s.cardLast4 }),
  outputMapper: (sub: any) => ({ txnId: sub.txnId }),
})
```
