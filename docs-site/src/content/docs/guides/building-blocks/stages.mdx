...
`.addFunction(name, fn, id, description?)` appends a stage that runs after the previous one (`id` is required).
...
`.addDeciderFunction(name, fn, id, description?)` adds a stage whose return value selects the next branch (`id` is required).
...
`.addSelectorFunction(name, fn, id, description?, options?)` adds a stage that selects **multiple** branches and runs them in parallel (`id` is required; `options` carries `{ failFast }`).
