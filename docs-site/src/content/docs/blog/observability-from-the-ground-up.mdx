...
## What we capture: three channels, one causal trace

FootPrint observes execution along **three orthogonal channels**. Each channel has a single purpose. Together they reconstruct the full causal trace of a run.
...
interface Recorder {
  ...
  onCommit?(event: { writes, runtimeStageId, ... }): void;
  onStageStart?(event): void;
  onStageEnd?(event): void;
  onError?(event): void;
}
```
