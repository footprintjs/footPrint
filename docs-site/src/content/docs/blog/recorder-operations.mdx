...
<Aside type="note">
  The abstract `KeyedRecorder<T>` / `SequenceRecorder<T>` base classes were removed in 7.0.0 — compose a `KeyedStore<T>` / `SequenceStore<T>` field instead. Same three operations, no inheritance (composition is the only model).
</Aside>
