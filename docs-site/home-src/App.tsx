...
import {
  ChevronRight, ExternalLink, Sun, Moon, Menu, X,
  BookOpen, Play, Boxes, Code2,
} from "lucide-react";
...
function SyntaxBlock({ children, className }: { children: string; className?: string }) {
  const tokenize = (code: string): ReactNode[] => {
    const rules: [RegExp, string][] = [
      ...
      [/\b(flowChart|FlowChartExecutor|decide|enableNarrative|getNarrative|attachRecorder|MetricRecorder|narrative|addFunction|addDeciderFunction|addFunctionBranch|setDefault|end|build|run|scope|executor|toMCPTool)\b/, "dark:text-blue-300 text-blue-600"],
    ];
...
// ═══════════════════════════════════════════════════════════════════
// PROBLEM SECTION (4 steps + loan story + MCP)
// ═══════════════════════════════════════════════════════════════════

const STEPS = [
  {
    num: 1,
    title: "You write business logic",
    body: "A pipeline validates an order, checks risk, and routes to the right path. Normal TypedScope code — no instrumentation.",
...
const chart = flowChart<OrderState>('Validate', async (scope) => {
  ...
  {
    num: 2,
    title: "Decisions are declared, not buried",
    body: "Instead of if/else chains, express rules as data. The engine evaluates them — and records which matched and why.",
...
chart.addDeciderFunction('Route', (scope) => {
  ...
  {
    num: 3,
    title: "The trace writes itself",
    body: "Attach a recorder and run. Every stage, decision, and branch produces a human-readable narrative — automatically.",
...
// [Condition]: tier "premium" eq "premium" ✓ → express
// Stage 3: Next, it moved on to ExpressShip.`,
  },
  {
    num: 4,
    title: "AI backtracks the trace to explain",
    body: 'Loan pipeline rejects Bob. User asks: "Why was I rejected?" The LLM reads the trace — not the source code — and explains the causal chain.',
    code: `// The trace shows exactly what happened:
//
// Stage 1: Write creditScore = 580, dti = 0.6
// Stage 2: Write riskTier = "high", riskFactors = (3 items)
// [Condition]: riskTier "high" eq "high" ✓ → Reject
// Stage 4: Write decision = "REJECTED — below-average
//   credit; DTI exceeds 43%; Self-employed < 2yr"
//
// LLM backtracks: decision ← riskTier ← dti ← monthlyDebts
// "Your application was rejected because your DTI of 60%
//  exceeds the 43% maximum..." — from the trace, not imagination.`,
  },
  {
    num: 5,
    title: "One line to expose as an AI tool",
    body: "Any flowchart becomes an MCP tool automatically. Description, input schema, and step list are generated from the graph.",
    code: `const tool = chart.toMCPTool();
// {
//   name: 'assesscredit',
  ...
  return (
    <section className="py-20">
      <SectionTitle
        id="problem"
        badge="The Problem"
        title="Why traces should generate themselves"
        sub="Manual logging drifts. Structured traces don't."
      />
          ...
          <div key={s.num} className="grid md:grid-cols-2 gap-8 items-start">
            <div>
              <div className="flex items-center gap-3 mb-3">
                <span className="w-8 h-8 rounded-full bg-primary/15 text-primary text-sm font-bold flex items-center justify-center">
                  {s.num}
                </span>
  ...
  return (
    <section className="py-20">
      <SectionTitle
        id="how"
        badge="Interactive"
        title="Watch the recorder in action"
        sub="Click play to step through an order-processing flowchart. The narrative writes itself at each stage."
      />
          ...
          <button
            ...
            className={cn(
              ...
              i <= active
                ...
                : "dark:border-zinc-700 border-zinc-300 dark:text-zinc-400 text-zinc-500"
            )}
          >
            {s.label}
          </button>
        ))}
        <button onClick={play} className="ml-4 p-2 rounded-full bg-primary/15 text-primary hover:bg-primary/25 transition" aria-label="Play demo">
...
// ═══════════════════════════════════════════════════════════════════
// QUICK START — real footprintjs API
// ═══════════════════════════════════════════════════════════════════
  ...
  return (
    <section className="py-20">
      <SectionTitle
        ...
        sub="Install, define your state, build a chart, attach a recorder."
      />
      <div className="space-y-6 max-w-3xl mx-auto">
        ...
        <div>
          <h3 className="text-sm font-semibold mb-2 dark:text-zinc-400 text-zinc-500">2 · Define state + build chart</h3>
          <SyntaxBlock>{`import { flowChart, FlowChartExecutor, decide } from 'footprintjs';
...
const chart = flowChart<OrderState>('Validate', async (scope) => {
  ...
  .addDeciderFunction('Route', (scope) => {
    return decide(scope, [
      { when: { tier: { eq: 'premium' } }, then: 'express', label: 'Premium' },
    ], 'standard');
  }, 'route')
...
executor.enableNarrative();
await executor.run({ input: { orderId: 'ORD-001', total: 149 } });
...
// Stage 1: The process began with Validate.
//   Step 1: Write tier = "premium"
// [Condition]: tier "premium" eq "premium" ✓ → chose express.
// Stage 3: Next, it moved on to ExpressShip.
//   Step 1: Write status = "express-shipping"`}</SyntaxBlock>
        </div>
      </div>
    </section>
  );
  ...
  const cards = [
    {
      ...
      title: "Documentation",
      desc: "Getting started guide, key concepts, and building blocks.",
      href: "https://footprintjs.github.io/footPrint/getting-started/quick-start/",
      external: true,
    },
    {
      icon: <Play className="w-5 h-5" />,
      title: "Interactive Playground",
      desc: "Build and run flowcharts in the browser with live narrative output.",
      href: "https://footprintjs.github.io/footprint-playground/",
      external: true,
    },
    {
      icon: <Code2 className="w-5 h-5" />,
      title: "Examples",
      desc: "31 runnable examples: building blocks, features, flow recorders, errors.",
      href: "https://github.com/footprintjs/footPrint/tree/main/examples",
      external: true,
    },
    {
      icon: <Github className="w-5 h-5" />,
      title: "GitHub",
      desc: "Source, issues, discussions, and contribution guide.",
    ...
    },
    {
      icon: <BookOpen className="w-5 h-5" />,
      title: "API Reference",
      desc: "Full TypeDoc reference for every class, function, and type.",
      href: "https://footprintjs.github.io/footPrint/api/",
      external: true,
    },
    {
      icon: <Boxes className="w-5 h-5" />,
      title: "npm",
      desc: "Install footprintjs. ESM + CommonJS. Zero dependencies.",
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold group-hover:text-primary transition-colors text-sm">{c.title}</h3>
              </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">
              <span className="font-semibold uppercase">FOOTPRINT</span><span className="text-primary font-semibold text-xs">js</span>
            </span>
          </div>
