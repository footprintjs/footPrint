...
const METHOD_ROUTES = {
    ...
    $read: (t) => (dotPath) => {
        ...
        if (!dotPath.includes('.'))
            return value;
        return (0, pathOps_js_1.nativeGet)(value, dotPath.slice(rootKey.length + 1));
    },
...
function createTerminalProxy(obj, rootKey, segments, target, state, visited = new Set()) {
    ...
    return new Proxy(obj, {
        get(raw, prop) {
            ...
            if ((0, allowlist_js_1.shouldWrapWithProxy)(value) && !Array.isArray(value) && !visited.has(value)) {
        ...
        set(raw, prop, value) {
            ...
            const patch = (0, pathBuilder_js_1.buildNestedPatch)(childSegments, unwrapProxy(value));
...
function createNestedProxy(obj, rootKey, segments, target, readSilent, state, ancestors = new Set()) {
    return new Proxy(obj, {
        get(raw, prop) {
            ...
            if (!(0, allowlist_js_1.shouldWrapWithProxy)(value))
                return value;
            const childSegments = [...segments, prop];
            // Array -- return array proxy
            if (Array.isArray(value)) {
                return (0, arrayTraps_js_1.createArrayProxy)(() => {
                    var _a;
                    const current = readSilent(rootKey);
                    return (_a = (0, pathOps_js_1.nativeGet)(current, childSegments.join('.'))) !== null && _a !== void 0 ? _a : [];
                }, (newArr) => {
                    const patch = (0, pathBuilder_js_1.buildNestedPatch)(childSegments, unwrapProxy(newArr));
                    target.updateValue(rootKey, patch);
                    state.childCache.delete(rootKey);
                });
            }
 ...
 */
function createTypedScope(target, options) {
    var _a;
    const state = {
    ...
    const proxy = new Proxy(target, {
        get(_proxyTarget, prop, _receiver) {
            ...
            if (prop === types_js_1.IS_TYPED_SCOPE)
                return true;
            if (prop === types_js_1.BREAK_SETTER) {
                return (fn) => {
                    state.breakFn = fn;
                };
            }
            ...
            if (types_js_1.SCOPE_METHOD_NAMES.has(prop)) {
            ...
            if (types_js_1.EXECUTOR_INTERNAL_METHODS.has(prop) && typeof target[prop] === 'function') {
            ...
            if (Array.isArray(value)) {
                ...
                if (cached && cached.ref === value)
                    return cached.proxy;
                const arrProxy = (0, arrayTraps_js_1.createArrayProxy)(() => { var _a; return (_a = readSilent(prop)) !== null && _a !== void 0 ? _a : []; }, (newArr) => {
